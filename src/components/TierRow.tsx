...
export default function TierRow({ level, drinks }: { level: TierLevel; drinks: Drink[] }) {
    ...
    return (
        <div className={`flex h-[112px] md:h-[120px] border-b-4 border-black`}>
            {/* 等级标签区域 */}
            ...
            <div className="flex-grow bg-white overflow-x-auto p-1 pb-2 md:p-2 flex space-x-2 md:space-x-3">
                {drinks.length > 0 ? (
                    ...
                    >
                        {/* 饮料图片占位 */}
                        <div className="bg-neutral-300 w-24 h-24 border-2 border-black shadow-neo group-hover:shadow-none group-hover:translate-x-0.5 group-hover:translate-y-0.5 transition-all duration-200">
                            {/* 实际项目中替换为真实图片 */}
                        </div>
